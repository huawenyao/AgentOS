...
# -*- coding: utf-8 -*-
"""
EFIAgent Phase 2 Windows兼容演示脚本
展示企业级多智能体协作系统的核心功能
"""
...
class EFIAgentDemo:
    ...
    async def initialize_system(self):
        ...
        # 创建通信框架
        print("初始化多模态协作通信框架...")
        self.communication_framework = CollaborativeFramework()
    ...
    async def demonstrate_skill_matching(self):
        ...
        for i, match in enumerate(matches, 1):
            agent = next((a for a in self.agents if a.id == match["agent_id"]), None)
            if agent:
                print(f"   {i}. {agent.name} - 相似度: {match['similarity']:.3f} - 匹配技能: {', '.join(match['matched_skills'])}")
            else:
                print(f"   {i}. 未知智能体 - 相似度: {match['similarity']:.3f} - 匹配技能: {', '.join(match['matched_skills'])}")
